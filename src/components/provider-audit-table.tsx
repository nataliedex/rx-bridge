...
import { markProviderPriceVerified, removeProviderMedicationPrice } from "@/lib/actions";
import { formatCurrency } from "@/lib/pricing";
import { UpdateProviderPriceDrawer } from "./update-provider-price-drawer";
import { ConfirmAction } from "./confirm-action";

import type { ProviderAuditStatus } from "@/lib/actions";
...
const STATUS_STYLES: Record<ProviderAuditStatus, { label: string; bg: string; text: string }> = {
...
function buildUrl(params: Record<string, string>) {
  ...
  return `/network/provider-audit${qs ? `?${qs}` : ""}`;
}

export function ProviderAuditTable({
  rows, currentFilter, currentSearch, currentProvider, currentMedication, providers, medications,
}: Props) {
  const router = useRouter();
  const [search, setSearch] = useState(currentSearch);
  const [verifying, setVerifying] = useState<string | null>(null);
  const [justVerified, setJustVerified] = useState<string | null>(null);
  const [confirmRemove, setConfirmRemove] = useState<{ medicationId: string; medicationName: string; providerId: string; providerName: string } | null>(null);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      if (search !== currentSearch) {
        router.push(buildUrl({ filter: currentFilter, search, provider: currentProvider, medication: currentMedication }));
      }
    }, 300);
  ...
  function navigate(overrides: Record<string, string>) {
    router.push(buildUrl({ filter: currentFilter, search, provider: currentProvider, medication: currentMedication, ...overrides }));
  }
  ...
  async function handleVerify(e: React.MouseEvent | null, priceEntryId: string) {
    ...
    try {
      await markProviderPriceVerified(priceEntryId);
  ...
  async function handleRemove() {
    ...
    const key = `${confirmRemove.medicationId}:${confirmRemove.providerId}`;
    setRemovingKey(key);
    try {
      await removeProviderMedicationPrice(confirmRemove.providerId, confirmRemove.medicationId);
  ...
  const hasFilters = currentSearch || currentProvider || currentMedication;
    ...
    <>
      ...
      <div className="shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3">
          <div className="relative">
            <input
              ...
              placeholder="Search medication or provider..."
          ...
          <select
            ...
            onChange={(e) => navigate({ provider: e.target.value })}
        ...
        {confirmRemove && (
          <div className="mb-3">
            <ConfirmAction
              message={`Remove ${confirmRemove.medicationName} pricing from ${confirmRemove.providerName}?`}
              detail="Active pricing will be deactivated. Historical records are preserved."
              confirmLabel="Remove"
              onConfirm={handleRemove}
              onCancel={() => setConfirmRemove(null)}
              loading={removingKey === `${confirmRemove.medicationId}:${confirmRemove.providerId}`}
            />
          </div>
        )}
      </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {rows.map((row, idx) => {
              ...
              return (
                <div
                  key={`${row.medicationId}:${row.providerId}`}
