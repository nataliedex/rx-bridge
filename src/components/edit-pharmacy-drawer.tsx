...
import { updatePharmacy, archivePharmacy, unarchivePharmacy } from "@/lib/actions";
...
export interface EditablePharmacy {
  id: string;
  name: string;
...
export function EditPharmacyDrawer({ open, onClose, pharmacy }: Props) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setContactName(pharmacy.contactName ?? "");
      setStreet(pharmacy.street ?? "");
      setCity(pharmacy.city ?? "");
      setState(pharmacy.state ?? "");
      setZip(pharmacy.zip ?? "");
      setServiceStates(parseServiceStates(pharmacy.serviceStates));
  ...
  async function handleSave() {
    ...
    try {
      await updatePharmacy(pharmacy.id, { name, street, city, state, zip, serviceStates, email, phone, contactName });
      setSuccess(true);
      router.refresh();
  ...
  async function handleArchive() {
    setArchiving(true);
    try {
      await archivePharmacy(pharmacy.id);
      router.refresh();
      onClose();
    } catch (err) {
  ...
  async function handleUnarchive() {
    setArchiving(true);
    try {
      await unarchivePharmacy(pharmacy.id);
      router.refresh();
      onClose();
    } catch (err) {
  ...
  const isArchived = !!pharmacy.archivedAt;

  return (
    <Drawer open={open} onClose={onClose} title="Edit Pharmacy">
      {success ? (
        <div className="flex items-center gap-2 text-green-700 text-sm">
          <span>&#10003;</span> Pharmacy updated
        </div>
      ) : (
        <div className="space-y-4" onKeyDown={handleKeyDown}>
          ...
          {isArchived && (
            <div className="bg-gray-50 border border-gray-200 rounded-md px-3 py-2 flex items-center justify-between">
              <span className="text-xs text-gray-500">This pharmacy is archived</span>
            ...
            {statePickerOpen && (
              <div className="mt-2 border border-gray-200 rounded-md p-2 bg-gray-50">
                <div className="flex items-center gap-3 mb-2">
                  <button type="button" onClick={() => setServiceStates([...US_STATES])} className="text-[10px] text-indigo-600 hover:text-indigo-800 font-medium">Select all</button>
                  <button type="button" onClick={() => setServiceStates([])} className="text-[10px] text-gray-500 hover:text-gray-700 font-medium">Clear all</button>
          ...
          {/* Archive section — secondary, separated */}
          {!isArchived && (
            <div className="pt-4 border-t border-gray-100">
              {confirmArchive ? (
                <ConfirmAction
                  message="Archive this pharmacy?"
                  detail={
                    (pharmacy.activePriceCount > 0
                      ? `${pharmacy.activePriceCount} active price${pharmacy.activePriceCount !== 1 ? "s" : ""} will be deactivated. `
                      : "") + "Historical records will be preserved."
                  }
                  confirmLabel="Archive Pharmacy"
