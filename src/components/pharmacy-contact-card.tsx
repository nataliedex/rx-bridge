...
import { updatePharmacyContact } from "@/lib/actions";
...
export function PharmacyContactCard({ pharmacyId, contactName, phone, email, fax }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      await updatePharmacyContact(pharmacyId, {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      {editing ? (
        <div className="space-y-3">
          ...
          <div>
            ...
            <input type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)}
              placeholder="orders@pharmacy.com" className={inputClass} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Fax</label>
            <input type="tel" value={formatPhone(editFax)} onChange={(e) => setEditFax(e.target.value.replace(/\D/g, "").slice(0, 10))}
              placeholder="(555) 123-4568" className={inputClass} />
          </div>
        ...
        <div className="space-y-2">
          ...
          {fax && <ViewRow label="Fax" value={formatPhone(fax)} />}
          {!contactName && !phone && !email && !fax && (
